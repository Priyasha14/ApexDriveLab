...
from ai.neural_policy import NeuralPolicy
...
def run_neural_laps(
    model_path: Path = Path("models") / "neural_policy.npz",
    laps: int = 1,
    ...
    samples = 0
    max_grip = 0.0
    driver_state = DriverState(decision="neural")

    while elapsed < max_time and checkpoints.state.lap_count < laps:
        inputs = policy.control(car, track, racing_line)
    ...
    if save_telemetry:
        telemetry.save_csv(telemetry_path or Path("runs") / "neural_lap.csv")

    return NeuralLapResult(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the trained neural policy in headless simulation.")
    parser.add_argument("--model", type=Path, default=Path("models") / "neural_policy.npz")
    ...
    result = run_neural_laps(model_path=args.model, laps=args.laps, max_time=args.max_time, setup_name=args.setup, save_telemetry=args.save_telemetry)
    print(result)
